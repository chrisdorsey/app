...
(function($) {
	...
	var FORMALIZE = {
		...
		createForm 			: function(form_config)				{
			...
			$.each(form_config.elements, function( index, form_field ){
					if($.isset(form_field.attributes)){
						console.log("Loading Form Field " + form_field.type + " " + form_field.attributes.name);
					}
					else {
						console.log("Loading Form Field " + form_field.type + " " + form_field.name);	
					}
					if (form_field.type == "radio" || form_field.type == "checkbox"){
						var input_id 		= form_id + "_" + form_field.name;
					} else {
						var input_id 		= form_id + "_" + form_field.attributes.name;
					};
					...
					if ( form_field.type == "textarea"){
						var input_html = FORMALIZE.createTextArea(input_id,form_field)
						container_html.append(label_html);
						container_html.append(input_html);
					}
		...
		createTextArea 		: function(input_id,form_field)		{
			//console.log("Creating Input");
			var default_input = this.default_input;
			form_field = $.extend(true,{},default_input,form_field);
			var data_attr = '';
			...
			var input_html = $('<textarea type = "'+ form_field.type +'" '+ data_attr +' ></textarea>').attr("id",input_id);
			//console.log(input_html);
			$.each(form_field.attributes,function( key , value ){
					if(key == value){
						input_html.append(value)} 
					else  {
						input_html.attr(key,value);
					};
			});
		...
		setData				: function(element_id,data){
				$.each(data,function(key,value){
					//console.log(key);
					
					$('#'+element_id).data(key,value);
					var element_data = $("#" +element_id).data(key);
					//console.log( key +":" + element_data);
				});
		},
